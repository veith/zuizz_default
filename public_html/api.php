...
if ($_REQUEST ['ZU_feature']) {
    ...
    if(isset($_REQUEST['ZU_identifier'])){
        // split
        $ex = explode('/',$_REQUEST['ZU_identifier']);
        $count = count($ex);
        if($count % 2 == 0){
            // submod => id, submod => id
            for ($i = 0; $i < $count; $i+=2) {
                $_REQUEST ['ZU_feature'] .= ".{$ex[$i]}";
                $ident[] = $ex[$i+1];
            }
            if(count($ident) == 1){
                $_REQUEST['ZU_identifier'] = $ident[0];
            }else{
                $_REQUEST['ZU_identifier'] = $ident;
            }


        }else{
            // id , submod => id , submod => id
            for ($i = 0; $i < $count; $i+=2) {
                if($i+1 < $count){
                    $_REQUEST ['ZU_feature'] .= ".{$ex[$i+1]}";
                }
                $ident[] = $ex[$i];
            }
            if(count($ident) == 1){
    ...
    $parameter ['feature'] = $_REQUEST ['ZU_feature'];
